...
export async function POST(request: Request) {
  try {
    ...
    const rag_url = new URL(
      process.env.RAG_RETRIEVAL_ENDPOINT ||
        "http://127.0.0.1:8000/api/v1/retrieval-augmented-generations"
    );
    ...
    const response_url = new URL(
      process.env.GENERATE_RESPONSE_ENDPOINT ||
        "http://127.0.0.1:8000/api/v1/llm-response"
    );
